...
class HumanoidModel:
    def __init__(self):
        # Load URDF model
        self.model_path = str(Path(__file__).parent /
    ...
    def calculate_pose_difference(self, target_angles):
        ...
        for joint_name in self.joint_names:
            if joint_name in target_angles:
                ...
                differences[joint_name] = np.degrees(
                    diff)  # Convert to degrees
        return differences

    def update_pose(self, joint_angles):
        """Update the humanoid model's pose based on joint angles
        Args:
            joint_angles: Dictionary mapping joint names to angle values
        """
